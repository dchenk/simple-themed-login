...
<form id="setupform" method="post" action="<?php $template->the_action_url( 'register', 'login_post' ); ?>">
	...
	<label for="blogname<?php $template->the_instance(); ?>"><?php _e( 'Site Name:', 'themed-login' ); ?></label>
	<?php } else { ?>
	<label for="blogname<?php $template->the_instance(); ?>"><?php _e( 'Site Domain:', 'themed-login' ); ?></label>
	<?php } ?>
	...
	<?php if ( ! is_user_logged_in() ) {
		if ( ! is_subdomain_install() )
			$site = $current_site->domain . $current_site->path . __( 'sitename', 'themed-login' );
		else
			$site = __( 'domain', 'themed-login' ) . '.' . $site_domain . $current_site->path;
		echo '<p>(<strong>' . sprintf( __( 'Your address will be %s.', 'themed-login' ), $site ) . '</strong>) ' . __( 'Must be at least 4 characters, letters and numbers only. It cannot be changed, so choose carefully!', 'themed-login' ) . '</p>';
	} ?>

	<label for="blog_title<?php $template->the_instance(); ?>"><?php _e( 'Site Title:', 'themed-login' ); ?></label>
	...
	<div id="privacy">
		<p class="privacy-intro">
			<label for="blog_public_on<?php $template->the_instance(); ?>"><?php _e( 'Privacy:', 'themed-login' ); ?></label>
			<?php _e( 'Allow search engines to index this site.', 'themed-login' ); ?>
			<br style="clear:both">
			<label class="checkbox" for="blog_public_on<?php $template->the_instance(); ?>">
				...
				<strong><?php _e( 'Yes', 'themed-login' ); ?></strong>
			</label>
			<label class="checkbox" for="blog_public_off<?php $template->the_instance(); ?>">
				...
				<strong><?php _e( 'No', 'themed-login' ); ?></strong>
			</label>
		</p>
	</div>
	...
	<p class="submit"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e( 'Signup', 'themed-login' ); ?>"></p>
</form>
